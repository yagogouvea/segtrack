...
router.post('/', (0, auth_middleware_1.requirePermission)('create:foto'), upload.single('foto'), async (req, res) => {
    ...
    try {
        ...
        if (arquivo) {
            const filepath = path_1.default.join(UPLOAD_DIR, arquivo.filename);
...
router.post('/test', upload.single('foto'), async (req, res) => {
...
router.post('/test-auth', upload.single('foto'), async (req, res) => {
