...
router.use(auth_middleware_1.authenticateCliente);
...
router.get('/cliente/ocorrencias', async (req, res) => {
    try {
        ...
        const nomeCliente = (0, diacritics_1.remove)(cliente.razaoSocial || '').toLowerCase().replace(/\s+/g, '');
        ...
        const ocorrenciasCliente = ocorrencias.filter((o) => {
            const nomeOcorrencia = (0, diacritics_1.remove)(o.cliente || '').toLowerCase().replace(/\s+/g, '');
        ...
        if (ocorrenciasCliente.length > 0) {
            console.log('📋 Primeiras ocorrências do cliente:', ocorrenciasCliente.slice(0, 3).map((o) => ({
                id: o.id,
                placa: o.placa1,
                cliente: o.cliente,
                status: o.status,
...
router.get('/cliente/estatisticas', async (req, res) => {
    try {
        ...
        const emAndamento = ocorrenciasCliente.filter((o) => o.status === 'em_andamento').length;
        ...
        const apropriacao = ocorrenciasCliente.filter((o) => o.tipo === 'apropriação' || o.tipo === 'apropriacao' || o.tipo === 'Apropriação').length;
        const estatisticas = {
            totalOcorrencias,
            emAndamento,
            recuperadas,
